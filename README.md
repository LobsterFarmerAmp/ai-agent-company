...
- **Company OS**--**自研**独立插件（非 OpenClaw 内置/官方），统一提供组织管理、任务流、会议编排、告示板
...
6. **手册非资产库**：仓库只做说明手册，个人/公司私有资产（头像、密钥、内部资料）不入库，只记录路径与规则
...
- **Company OS**（**自研**独立插件，代码在 `~/.openclaw/company/openclaw-plugin-company-os`，需配置 `plugins.load.paths` 加载）：`company_task_*`（任务流）、`company_meeting_*`（会议编排）、`company_notice_*`（告示板）、`company_org_*`（组织管理）、`company_inbox`（统一收件箱）
